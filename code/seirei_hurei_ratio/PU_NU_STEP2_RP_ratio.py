...
if len(df_seirei)<=len(df_hurei_reliable):
    ...
    for i in range(1):
        num = i+1
        df_hurei_reliable = df_hurei_reliable.sample(frac=1)
        exec("df_hurei_{} = df_hurei_reliable".format(num))
else:
    print("575行目でエラー")
...
if len(df_seirei_more_reliable)<=len(df_hurei_reliable):
    ...
    for i in range(1):
        num = i+1
        df_hurei_reliable = df_hurei_reliable.sample(frac=1)
        exec("df_more_hurei_{} = df_hurei_reliable".format(num))
else:
...
for i in range(1):
  ...
  save_path = "../model/hozon_sarcasm_detection_PU_NU_raito" + str(num)
...
av_Accuracy = sum(Accuracy_list)
av_Precision = sum(Precision_list)
av_Recall = sum(Recall_list)
av_F1_score = sum(F1_score_list)
