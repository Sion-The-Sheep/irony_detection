# bert_hannkyousi_bunnruiki_1_RN_ver4.py に対応

#hannkyousi_bunnruiki_1で作成したモデルを使って、しきい値を定める
#ランダムに集めたツイートのうち、皮肉である確率がしきい値を下回るデータをＲＮとして抽出
#皮肉がタグ付けされたデータを正例、ＲＮを負例として学習して新たな分類器２を作成する

#ver0：スパイを一度選出し、信頼できる負例を抽出して利用する
#ver1：スパイを複数回選出し、それぞれのパターン全てで信頼できる負例とみなされたものを最終的な信頼できる負例として利用する機能追加
#ver2：最終的な分類器を作成するとき負例として用いるデータを、信頼できる負例データからランダムに数回取ってくるように変更（変更前はインデックス順に区切ってとってきてる）
#ver3: 閾値b％以下のものを確かな負例データとする操作とは別に、各スパイデータで信頼できる負例である確率が高いと判断されたデータを集め、信頼できる負例データとするコードの追加
...
#path_spy = "~/soturon/tweetdata_random_include_spy.csv"
...
for q in range(5):
    ...
    path_spy = "~/shuusi/data/step1/tweetdata_random_include_spy_no_zengo" + str(spy_pattern) + ".csv"
    ...
    model_name = "/home/aquamarine/sion/shuusi/model/hozon_spy_RN_no_zengo" + str(spy_pattern) + "/model_epoch2.model"
    ...
    for i in range(len(df)):
        if label_lst[i] == 1 and new_label_lst[i] == 0:
    ...
    df = pd.read_csv(path9_BIG_MC,encoding="utf_8",names=('honbun','rep','inyou','label'),dtype=object)########################################################################################################################################
    ...
    for input_ids, attention_mask,token_type_ids in r_dl:
        ...
        ver3_list.append((Count,r_soft[count,1].item()))#ver3新規
        Count += 1
...
df = pd.read_csv(path9_BIG_MC,encoding="utf_8",names=('honbun','rep','inyou','label'))####################################################################################################################################################
...
print("信頼できる負例数：",len(df_hurei_reliable))
print("とても信頼できる負例数：",len(df_hurei_more_reliable))
...
#作成した信頼できる負例データを保存
#df_hurei_reliable.to_csv("~/soturon/tweetdata_hurei_reliable.csv",header=False,index=False,encoding="utf-8")
df_hurei_reliable.to_csv("../data/step1/tweetdata_hurei_reliable_no_zengo.csv",header=False,index=False,encoding="utf-8")

#作成したとても信頼できる負例データを保存 
#df_hurei_more_reliable.to_csv("~/soturon/tweetdata_hurei_more_reliable.csv",header=False,index=False,encoding="utf-8")
df_hurei_more_reliable.to_csv("../data/step1/tweetdata_hurei_more_reliable_no_zengo.csv",header=False,index=False,encoding="utf-8")
...
df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)



print("信頼できる負例データ数：",len(df_hurei_reliable))
print("とても信頼できる負例データ数：",len(df_hurei_more_reliable))
print("正例データ数：",len(df_seirei))

num_seirei = len(df_seirei)

#信頼できる負例データから、正例データ数と同じ数だけランダムにデータを取ってくる操作を５回行い、５つの負例データを作成する

#2022/01/12
#5つではなく20個の負例データを作成
df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_1 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_2 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_3 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_4 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_5 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_6 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_7 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_8 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_9 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_10 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_11 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_12 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_13 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_14 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_15 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_16 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_17 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_18 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_19 = df_hurei_reliable[:num_seirei]

df_hurei_reliable = df_hurei_reliable.sample(frac=1)
df_hurei_20 = df_hurei_reliable[:num_seirei]

#とても信頼できる負例データも同様に 
df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_1 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_2 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_3 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_4 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_5 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_6 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_7 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_8 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_9 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_10 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_11 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_12 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_13 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_14 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_15 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_16 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_17 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_18 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_19 = df_hurei_more_reliable[:num_seirei]

df_hurei_more_reliable = df_hurei_more_reliable.sample(frac=1)
df_more_hurei_20 = df_hurei_more_reliable[:num_seirei]
...
#hurei_honbun_list_M1 = df_more_hurei_1['honbun'].tolist()
#hurei_rep_list_M1 = df_more_hurei_1['rep'].tolist()
#hurei_inyou_list_M1 = df_more_hurei_1['inyou'].tolist()
#hurei_label_list_M1 = df_more_hurei_1['label'].tolist()

########################################################

#hurei_honbun_list_M2 = df_more_hurei_2['honbun'].tolist()
#hurei_rep_list_M2 = df_more_hurei_2['rep'].tolist()
#hurei_inyou_list_M2 = df_more_hurei_2['inyou'].tolist()
#hurei_label_list_M2 = df_more_hurei_2['label'].tolist()

########################################################

#hurei_honbun_list_M3 = df_more_hurei_3['honbun'].tolist()
#hurei_rep_list_M3 = df_more_hurei_3['rep'].tolist()
#hurei_inyou_list_M3 = df_more_hurei_3['inyou'].tolist()
#hurei_label_list_M3 = df_more_hurei_3['label'].tolist()

########################################################

#hurei_honbun_list_M4 = df_more_hurei_4['honbun'].tolist()
#hurei_rep_list_M4 = df_more_hurei_4['rep'].tolist()
#hurei_inyou_list_M4 = df_more_hurei_4['inyou'].tolist()
#hurei_label_list_M4 = df_more_hurei_4['label'].tolist()

########################################################

#hurei_honbun_list_M5 = df_more_hurei_5['honbun'].tolist()
#hurei_rep_list_M5 = df_more_hurei_5['rep'].tolist()
#hurei_inyou_list_M5 = df_more_hurei_5['inyou'].tolist()
#hurei_label_list_M5 = df_more_hurei_5['label'].tolist()
...
for i in range(20):
    num = i+1
    exec("train_honbun_list_M{} = seirei_honbun_list + hurei_honbun_list_M{}".format(num,num))
    exec("train_rep_list_M{} = seirei_rep_list + hurei_rep_list_M{}".format(num,num))
    exec("train_inyou_list_M{} = seirei_inyou_list + hurei_inyou_list_M{}".format(num,num))
    exec("train_label_list_M{} = seirei_label_list + hurei_label_list_M{}".format(num,num))


#train_honbun_list_M1 = seirei_honbun_list + hurei_honbun_list_M1
#train_rep_list_M1 = seirei_rep_list + hurei_rep_list_M1
#train_inyou_list_M1 = seirei_inyou_list + hurei_inyou_list_M1
#train_label_list_M1 = seirei_label_list + hurei_label_list_M1

#######################################################

#train_honbun_list_M2 = seirei_honbun_list + hurei_honbun_list_M2
#train_rep_list_M2 = seirei_rep_list + hurei_rep_list_M2
#train_inyou_list_M2 = seirei_inyou_list + hurei_inyou_list_M2
#train_label_list_M2 = seirei_label_list + hurei_label_list_M2

#######################################################                                                                                                                                                                                                                          

#train_honbun_list_M3 = seirei_honbun_list + hurei_honbun_list_M3
#train_rep_list_M3 = seirei_rep_list + hurei_rep_list_M3
#train_inyou_list_M3 = seirei_inyou_list + hurei_inyou_list_M3
#train_label_list_M3 = seirei_label_list + hurei_label_list_M3

#######################################################                                                                                                                                                                                                                          

#train_honbun_list_M4 = seirei_honbun_list + hurei_honbun_list_M4
#train_rep_list_M4 = seirei_rep_list + hurei_rep_list_M4
#train_inyou_list_M4 = seirei_inyou_list + hurei_inyou_list_M4
#train_label_list_M4 = seirei_label_list + hurei_label_list_M4

#######################################################                                                                                                                                                                                                                         

#train_honbun_list_M5 = seirei_honbun_list + hurei_honbun_list_M5
#train_rep_list_M5 = seirei_rep_list + hurei_rep_list_M5
#train_inyou_list_M5 = seirei_inyou_list + hurei_inyou_list_M5
#train_label_list_M5 = seirei_label_list + hurei_label_list_M5
...
for i in range(5):
  ...
  #save_path = "hozon_reliable" + str(num) 2022/12/11
  save_path = "../model/hozon_sarcasm_detection_PU_no_zengo" + str(num)
...
#print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
#print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
#print("ここからとても信頼できる負例データを用いた交差検証開始")
#print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
#print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
...
#for i in range(5):
#  num = i+1
#  exec("x_train = train_honbun_list_M{}".format(num))
#  exec("x_train2 = train_rep_list_M{}".format(num))
#  exec("x_train3 = train_inyou_list_M{}".format(num))
#  exec("y = train_label_list_M{}".format(num))
#  y_train = torch.tensor(y, dtype=torch.int64)
...
#print("とても信頼できる負例を用いた半教師あり学習最終結果")
#print("正解率：",av_Accuracy,"適合率：",av_Precision,"再現率：",av_Recall,"F値：",av_F1_score)
...
print("ここから半教師なしで学習した結果を表示する")



df_hurei_random = pd.read_csv(path9_BIG_MC,encoding="utf_8",names=('honbun','rep','inyou','label'),dtype=object)##########################################################################################################################################

df_hurei_random = df_hurei_random.applymap(str)
df_hurei_random['label'] = df_hurei_random['label'].astype('int')


df_hurei_random = df_hurei_random.sample(frac=1)
print("シャッフルされるか確認")
print(df_hurei_random.index[:30])

#それぞれランダムに並び替える                                                                                                                                                                                                                                                      
df_seirei = pd.read_csv(path8_BIG_MC,encoding="utf_8",names=('honbun','rep','inyou','label'),dtype=object)####################################################################################################################################################
df_seirei = df_seirei.sample(frac=1)
df_hurei = df_hurei_random.sample(frac=1)



num_seirei = len(df_seirei)

#ランダム負例データから、正例データ数と同じ数だけランダムにデータを取ってくる操作を５回行い、５つの負例データを作成する                                                                                                                                                          


df_hurei = df_hurei.sample(frac=1)
df_hurei_1 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_2 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_3 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_4 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_5 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_6 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_7 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_8 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_9 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_10 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_11 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_12 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_13 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_14 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_15 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_16 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_17 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_18 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_19 = df_hurei[:num_seirei]

df_hurei = df_hurei.sample(frac=1)
df_hurei_20 = df_hurei[:num_seirei]

print("ランダム負例データの中から、異なるデータを抽出してるか確認")
print("hurei1のインデックス",df_hurei_1.index)
print("hurei2のインデックス",df_hurei_2.index)
print("hurei3のインデックス",df_hurei_3.index)
print("hurei4のインデックス",df_hurei_4.index)
...
#BERTで学習                                                                                                                                                                                                                                                                        
print("BERTに渡すデータ作成完了")
print("信頼できる負例データを用いない場合での交差検証開始")
...
for i in range(5):
  ...
  #save_path = "hozon_sarcasm_detection_baceline" + str(num)
  save_path = "../model/hozon_sarcasm_detection_baceline_no_zengo" + str(num)
