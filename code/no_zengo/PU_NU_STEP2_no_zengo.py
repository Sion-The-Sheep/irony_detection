#半教師あり学習 ステップ２
#信頼できる負例ＲＮと文中に「皮肉」を含む文を用いてファインチューニングを行う
#ＲＮの15%をstr_hinikuに混ぜて学習した分類器を作成する
#作成したモデルは"hozon_rp"ディレクトリ内に保存
#これはbert_hannkyousi_bunnruiki_1.pyを元に作成している。ここではseirei:信頼できる負例データ　random:文中に「皮肉」を含む文　として実行する
#str_hiniku は ラベル:2
#kakko_hinikuは　ラベル:3

#ver0：スパイを一度選出し、データに混ぜて保存
#ver1: スパイデータを複数回選出し、その数だけスパイ入りデータを保存
...
#withou_aisatu を用いるのかをここで選択

#df_seirei = pd.read_csv("/home/aquamarine/sion/shuusi/data/step1/tweetdata_hurei_reliable.csv",encoding="utf_8",names=('honbun','rep','inyou','label'),dtype=object)
df_seirei = pd.read_csv("../data/step1/tweetdata_hurei_reliable_without_aisatu_useCLUSTERING_no_zengo.csv",encoding="utf_8",names=('honbun','rep','inyou','label'),dtype=object)

df_random = pd.read_csv(path12_BIG_MC_4,encoding="utf_8",names=('honbun','rep','inyou','label'),dtype=object)#########################################################################################################################
...
for q in range(5):
    ...
    save_path = "~/shuusi/data/step2/tweetdata_str_hiniku_include_spy_no_zengo"  + str(spy_pattern) + ".csv"
    ...
    #2022/12/01
    #save_path = "hozon_rp"
    #save_path = "hozon_rp" + str(spy_pattern)
    save_path = "../model/hozon_spy_RP_no_zengo" + str(spy_pattern)
